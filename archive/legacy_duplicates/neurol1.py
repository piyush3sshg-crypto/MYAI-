...
class NeuralIntentClassifier:
    ...
    def train(self, epochs=300, learning_rate=0.15, verbose=False, validation_split=0.0):
        ...
        self.intent_to_index = {name: idx for idx, name in enumerate(self.intent_names)}
        ...
        validation_data = shuffled[:val_count] if val_count > 0 else []
        training_data = [(x, t) for x, t, _ in (shuffled[val_count:] if val_count > 0 else shuffled)]
        ...
        for epoch in range(epochs):
            ...
            for x, target_index in training_data:
                epoch_loss += self.network.train_step(x, target_index, learning_rate)
        ...
        def _accuracy(dataset):
            if not dataset:
                return None
            correct = sum(
                1 for x, target_index in dataset
                if self.network.predict_index(x)[0] == target_index
            )
            return correct / len(dataset)

        training_accuracy = _accuracy(training_data)
        validation_accuracy = _accuracy([(x, t) for x, t, _ in validation_data]) if validation_data else None
    ...
    def evaluate_calibration(self, examples=None, num_buckets=5):
        if not self.trained:
            raise RuntimeError("neural intent classifier has not been trained yet")
        examples = examples if examples is not None else self.raw_examples
        if not examples:
            return {"buckets": [], "calibration_error": None, "examples_evaluated": 0}
        bucket_edges = [i / num_buckets for i in range(num_buckets + 1)]
        buckets = [{"range": (bucket_edges[i], bucket_edges[i + 1]), "total": 0, "correct": 0} for i in range(num_buckets)]
        for true_label, text in examples:
            match = self.predict(text, threshold=0.0)
            bucket_index = min(int(match.score * num_buckets), num_buckets - 1)
            buckets[bucket_index]["total"] += 1
            if match.intent_name == true_label:
                buckets[bucket_index]["correct"] += 1
        total_examples = sum(b["total"] for b in buckets)
        weighted_error = 0.0
        for bucket in buckets:
            ...
            bucket_midpoint = (bucket["range"][0] + bucket["range"][1]) / 2
            weighted_error += (bucket["total"] / total_examples) * abs(bucket["accuracy"] - bucket_midpoint)
        return {
            "buckets": buckets,
    ...
    def from_dict(cls, data):
        ...
        classifier.raw_examples = [tuple(e) for e in data.get("raw_examples", [])]
